...
LightSymphonyiPort.prototype._discoveriPort = function (callback) {
  ...
  // Handle various valid responses from iPort devices
  var responseValid = function (response) {
    var valid1 = Buffer.from('YES');
    var valid2 = Buffer.from('iPORT HERE');

    return response.compare(valid1, 0, valid1.length, 0, Math.min(response.length, valid1.length)) === 0
      || response.compare(valid2, 0, valid2.length, 0, Math.min(response.length, valid2.length)) === 0;
  };
  ...
  discoverTimeout = setTimeout(function () { errorCallback(new Error('iPort discover timed out')); }, DISCOVER_TIMEOUT);
};
