...
var COMMAND_PORT = 10001;
var DISCOVER_PORT = 30718;
var DISCOVER_TIMEOUT = 1000;
var MAX_NO_AREAS = 29;
...
function LightSymphonyiPort(log, ipAddress) {
  this.log = log;
...
LightSymphonyiPort.prototype._discoveriPort = function (callback) {
  ...
  socket.on('message', function (response, remoteHost) {
    this.log('_discoveriPort message event, response = ' + response.toString());
  ...
  socket.send(discoverMessage, 0, discoverMessage.length, DISCOVER_PORT, broadcastAddress);
...
LightSymphonyiPort.prototype._sendCommand = function (command, callback) {
  var sendCommand = function () {
    this.log('Sending: ' + command + ' to ' + this._ipAddress);
    var socket = dgram.createSocket('udp4');
